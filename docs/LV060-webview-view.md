...
| 能力 | 适合做什么 | 局限 |
|------|-----------|------|
| `TreeDataProvider` | 树形列表（本项目 `src/views/` 用的就是这个） | 节点只能是 `TreeItem`，仅支持 `label`/`iconPath`/`description` 等固定字段 |
...
功能由一个核心文件 `src/views/chat-webview.ts` 承载，在 `src/extension.ts` 中注册，通过 `package.json` 声明视图。
...
| 文件 | 说明 |
|------|------|
| `src/views/chat-webview.ts` | Webview View 核心模块，含 `ChatWebviewViewProvider`、HTML 生成、双向通信、注册函数 |
