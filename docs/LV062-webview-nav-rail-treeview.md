...
`src/tree-views/` 下 6 个 TreeView provider 的数据来源、树深、是否可写各不相同：
...
新建 [src/tree-views/registry.ts](src/tree-views/registry.ts)，定义统一节点形状与 provider 契约：
...
### 1. fixed-data provider 改造（[src/tree-views/fixed-data-provider.ts](src/tree-views/fixed-data-provider.ts)）
...
### 2. chat provider 路由新消息（[src/tree-views/chat-webview-view.ts](src/tree-views/chat-webview-view.ts)）
...
**src/tree-views/fixed-data-provider.ts**（瘦身约 130 行）
...
| 文件 | 说明 |
|------|------|
| `src/tree-views/registry.ts` | `SnapNode` / `ViewAction` / `SnapshottableProvider` / registry |
...
| 文件 | 修改内容 |
|------|---------|
| `src/tree-views/fixed-data-provider.ts` | 改造为纯 `SnapshottableProvider`（快照 + id 版 CRUD），删除原生树相关 |
| `src/tree-views/chat-webview-view.ts` | `_handleMessage` 新增 requestViewList/requestSnapshot/viewAction 分支 |
